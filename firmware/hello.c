...
/*****************************************************************************/
/*
static u_int8 getSBoxValue(u_int8 num)
{
  return sbox[num];
}
*/
...
{
  unsigned i, j, k;
  u_int8 tempa[4]; // Used for the column/row operations
...
static void AES_init_ctx(u_int8 *RoundKey, const u_int8* key)
{
  KeyExpansion(RoundKey, key);
}
/*#if (defined(CBC) && (CBC == 1)) || (defined(CTR) && (CTR == 1))
static void AES_init_ctx_iv(struct AES_ctx* ctx, const u_int8* key, const u_int8* iv)
{
  KeyExpansion(ctx->RoundKey, key);
  memcpy_dumped (ctx->Iv, iv, AES_BLOCKLEN);
}
static void AES_ctx_set_iv(struct AES_ctx* ctx, const u_int8* iv)
{
  memcpy_dumped (ctx->Iv, iv, AES_BLOCKLEN);
}
#endif*/
...
static void AddRoundKey(u_int8 round, state_t* state, const u_int8* RoundKey)
{
  u_int8 i,j;
  for (i = 0; i < 4; ++i)
  {
...
static u_int8 Multiply(u_int8 x, u_int8 y)
{
  return (((y & 1) * x) ^
       ((y>>1 & 1) * xtime(x)) ^
       ((y>>2 & 1) * xtime(xtime(x))) ^
...
/*
static u_int8 getSBoxInvert(u_int8 num)
{
  return rsbox[num];
}
*/
...
static void AES_ECB_encrypt(u_int8 *RoundKey, u_int8* buf)
{
  // The next function call encrypts the PlainText with the Key using AES algorithm.
  Cipher((state_t*)buf, RoundKey);
}

static void AES_ECB_decrypt(u_int8 *RoundKey, u_int8* buf)
{
...
static void XorWithIv(u_int8* buf, const u_int8* Iv)
{
  u_int8 i;
...
static void AES_CBC_encrypt_buffer(struct AES_ctx *ctx, u_int8* buf, size_t length)
{
  size_t i;
  u_int8 *Iv = ctx->Iv;
...
static void AES_CBC_decrypt_buffer(struct AES_ctx* ctx, u_int8* buf, size_t length)
{
  size_t i;
  u_int8 storeNextIv[AES_BLOCKLEN];
  for (i = 0; i < length; i += AES_BLOCKLEN)
  {
    memcpy_dumped(storeNextIv, buf, AES_BLOCKLEN);
    InvCipher((state_t*)buf, ctx->RoundKey);
    XorWithIv(buf, ctx->Iv);
    memcpy_dumped(ctx->Iv, storeNextIv, AES_BLOCKLEN);
    buf += AES_BLOCKLEN;
  }
...
/*static void AES_CTR_xcrypt_buffer(struct AES_ctx* ctx, u_int8* buf, size_t length)
{
  u_int8 buffer[AES_BLOCKLEN];
  ...
  for (i = 0, bi = AES_BLOCKLEN; i < length; ++i, ++bi)
  {
    if (bi == AES_BLOCKLEN) // we need to regen xor compliment in buffer
    {
      ...
      memcpy_dumped(buffer, ctx->Iv, AES_BLOCKLEN);
...
void hello(void)
{
    ...
    u_int8 out[] = { 0xf3, 0xee, 0xd1, 0xbd, 0xb5, 0xd2, 0xa0, 0x3c, 0x06, 0x4b, 0x5a, 0x7e, 0x3d, 0xb1, 0x81, 0xf8 };
#elif defined(AES192)
    u_int8 key[] = { 0x8e, 0x73, 0xb0, 0xf7, 0xda, 0x0e, 0x64, 0x52, 0xc8, 0x10, 0xf3, 0x2b, 0x80, 0x90, 0x79, 0xe5,
                      0x62, 0xf8, 0xea, 0xd2, 0x52, 0x2c, 0x6b, 0x7b };
    u_int8 out[] = { 0xbd, 0x33, 0x4f, 0x1d, 0x6e, 0x45, 0xf2, 0x5f, 0xf7, 0x12, 0xa2, 0x14, 0x57, 0x1f, 0xa5, 0xcc };
#elif !defined(AES128)
    u_int8 key[] = { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };
    //u_int8 out[] = { 0x3a, 0xd7, 0x7b, 0xb4, 0x0d, 0x7a, 0x36, 0x60, 0xa8, 0x9e, 0xca, 0xf3, 0x24, 0x66, 0xef, 0x97 };
#endif
